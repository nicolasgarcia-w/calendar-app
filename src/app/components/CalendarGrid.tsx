...
export function PartnerCalendarGrid({ days }: { days: PublicDay[] }) {
  ...
  return (
    <div className="w-full max-w-sm mx-auto">
      <h2 className="text-center text-rose-700 font-semibold text-lg mb-3">July 2026</h2>
      {/* Weekday headers */}
      ...
      <div className="grid grid-cols-7 gap-1">
        ...
        {/* Empty cell after July 31 (Saturday would be next, we end on Friday) */}
        <div />
      </div>
    </div>
  )
...
export function AdminCalendarGrid({ days }: { days: AdminDay[] }) {
  ...
  return (
    <div className="w-full max-w-sm mx-auto">
      <h2 className="text-center text-slate-700 font-semibold text-lg mb-1">July 2026</h2>
      {/* Status summary */}
      <div className="flex justify-center gap-3 text-xs mb-3">
        <span className="text-rose-600">✓ {published} published</span>
        <span className="text-amber-600">✏️ {draft} draft</span>
        <span className="text-slate-400">○ {empty} empty</span>
      </div>
      {/* Weekday headers */}
      <div className="grid grid-cols-7 mb-1">
        {WEEKDAYS.map((d) => (
