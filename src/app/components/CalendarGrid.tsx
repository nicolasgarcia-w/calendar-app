...
export function PartnerCalendarGrid({ days }: { days: PublicDay[] }) {
  ...
  return (
    <div className="w-full mx-auto">
      <h2 className="text-center text-rose-700 font-semibold text-lg mb-3 tracking-wide">Julio 2026</h2>
      <div className="grid grid-cols-7 mb-2">
        {WEEKDAYS.map((d) => (
          <div key={d} className="text-center text-xs font-semibold text-rose-300 py-1 uppercase tracking-wider">
            {d}
          </div>
        ))}
      </div>
      <div className="grid grid-cols-7 gap-2">
        ...
        {Array.from({ length: 31 }, (_, i) => i + 1).map((n, idx) => {
          const day = byNumber[n]
          if (!day) return <div key={n} className="aspect-square" />
          return <PartnerDayCard key={n} day={day} index={JULY_START_INDEX + idx} />
        })}
        <div />
      </div>
    </div>
    ...
    <div className="w-full mx-auto">
      ...
      <div className="grid grid-cols-7 gap-2">
        ...
        {Array.from({ length: 31 }, (_, i) => i + 1).map((n, idx) => {
          const day = byNumber[n]
          if (!day) return <div key={n} className="aspect-square" />
          return <AdminDayCard key={n} day={day} index={JULY_START_INDEX + idx} />
        })}
        <div />
      </div>
    </div>
