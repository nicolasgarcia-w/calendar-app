...
export function PartnerDayCard({ day, index = 0 }: PartnerCardProps) {
  ...
  if (isOpen) {
    const icon = isCompleted ? '✓' : isOpened ? (dayNumber === 2 ? '🧩' : dayNumber === 3 ? '🃏' : dayNumber === 4 ? '🫙' : dayNumber === 5 ? '🎵' : dayNumber === 6 ? '📚' : dayNumber === 7 ? '🖼️' : dayNumber === 8 ? '🫙' : dayNumber === 9 ? '🟩' : dayNumber === 10 ? '🔑' : dayNumber === 11 ? '⭐' : '💌') : '🎁'
